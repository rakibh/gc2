...
class GlobalSearchRepository extends Repository
{
    ...
    public function search(string $query): array
    {
        ...
        $stmt = $this->db->prepare("
            SELECT 'equipment' as type, e.id, CONCAT(e.brand, ' ', e.model) as title, 
                   CONCAT(e.serial_number, ' - ', COALESCE(e.location, 'No Location'), ' ', COALESCE(n.ip_address, '')) as subtitle 
            FROM equipments e
        ...
        $stmt = $this->db->prepare("
            SELECT 'task' as type, t.id, t.title, 
                   CONCAT(t.status, ' | Priority: ', t.priority) as subtitle 
            FROM tasks t
        ...
        $stmt = $this->db->prepare("
            SELECT 'network' as type, n.id, n.ip_address as title, 
                   CONCAT(COALESCE(e.brand, ''), ' ', COALESCE(e.model, ''), ' (', COALESCE(e.serial_number, 'Unassigned'), ')') as subtitle 
            FROM network_info n
        ...
        // 5. Search Equipment Types & Blocks (Admin only for clarity, but searchable for all if needed)
        $stmt = $this->db->prepare("
